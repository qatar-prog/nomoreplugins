...
import net.runelite.client.plugins.pvphelper.ScriptCommand.ScriptCommandFactory;
...
public class PvPHelperHotkeyListener extends MouseAdapter implements KeyListener
{
	...
	@Inject
	private PvPHelperPlugin plugin;

	@Inject
	private PvPHelperConfig config;

	@Inject
	private PvPHelperOverlay overlay;
	...
	@Inject
	private PvPHelperHotkeyListener(final Client client, final PvPHelperConfig config, final PvPHelperPlugin plugin, final PvPHelperOverlay overlay)
	{
		this.client = client;
		this.config = config;
		this.plugin = plugin;
	...
	public void keyPressed(KeyEvent e)
	{
		...
		try
		{
			...
			executor.submit(() -> {
				...
				else if (key_code == this.config.key6().getKeyCode())
				{
					this.processCommands(this.config.key6_script(), this.client, this.config, this.plugin, this.overlay, this.configManager);
				}
				else if (key_code == this.config.key7().getKeyCode())
				{
					this.processCommands(this.config.key7_script(), this.client, this.config, this.plugin, this.overlay, this.configManager);
				}
				else if (key_code == this.config.key8().getKeyCode())
				{
					this.processCommands(this.config.key8_script(), this.client, this.config, this.plugin, this.overlay, this.configManager);
				}
	...
	private void processCommands(final String command, final Client client, final PvPHelperConfig config, final PvPHelperPlugin plugin, final PvPHelperOverlay overlay, final ConfigManager configManager)
	{
