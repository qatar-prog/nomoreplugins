...
@Singleton
public class PvPHelperOverlay extends Overlay
{
	private final Client client;
	private final PvPHelperPlugin pkToolsPlugin;
	...
	@Inject
	private PvPHelperOverlay(Client client, PvPHelperPlugin plugin, SpriteManager spriteManager, PvPHelperConfig config)
	{
		this.client = client;
	...
	public Dimension render(Graphics2D graphics)
	{
		...
		if (lastEnemy == null)
		{
			PvPHelperOverlay.lastEnemyLocation = null;
		}
		else
		{
			...
			if (WEAPON_INT > 0)
			{
				if (Arrays.asList(PvPHelperOverlay.MELEE_LIST).contains(WEAPON_INT))
				{
					if (config.prayerHelper())
					{
						...
						if (!PROTECT_MELEE)
						{
							imagePanelComponent.setBackgroundColor(PvPHelperOverlay.NOT_ACTIVATED_BACKGROUND_COLOR);
						}
					}
				}
				else if (Arrays.asList(PvPHelperOverlay.RANGED_LIST).contains(WEAPON_INT))
				{
					if (config.prayerHelper())
					{
				...
				else if (Arrays.asList(PvPHelperOverlay.MAGIC_LIST).contains(WEAPON_INT))
				{
					if (config.prayerHelper())
					{
	...
	static final Integer[] MELEE_LIST = {ItemID.DRAGON_SCIMITAR, ItemID.RUNE_SCIMITAR, ItemID.GRANITE_MAUL,
		ItemID.ABYSSAL_WHIP, ItemID.ABYSSAL_WHIP_4178, ItemID.DORGESHUUN_CROSSBOW, ItemID.BANDOS_GODSWORD, ItemID.ARMADYL_GODSWORD,
		ItemID.VOLCANIC_ABYSSAL_WHIP, ItemID.FROZEN_ABYSSAL_WHIP, ItemID.BARRELCHEST_ANCHOR, ItemID.ABYSSAL_WHIP_4178,ItemID.ABYSSAL_WHIP_20405,
